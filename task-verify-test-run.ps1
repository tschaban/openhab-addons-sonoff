...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Maven is available" -ForegroundColor Green
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "[SUCCESS] All tests passed! JUnit 5 framework is properly configured." -ForegroundColor Green
        } else {
            Write-Host "[ERROR] Tests failed. Check the output above for details." -ForegroundColor Red
            exit 1
        }
    } else {
    ...
    Write-Host "[OK] JUnit 5 framework has been properly configured in pom.xml" -ForegroundColor Green
    Write-Host "[OK] Test classes are available in src/test/java/" -ForegroundColor Green
    Write-Host ""
